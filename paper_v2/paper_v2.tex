...
\begin{remark}
The absolute-continuity hypothesis on the endpoint coupling is automatic within the framework of this paper: the Doob $h$-transform construction of Section~\ref{sec:modified-levy} already requires $\boldsymbol{\Psi}$ to be absolutely continuous with respect to the terminal law of the driver, and the initial value is a point of the driver's support. The lemma uses only the Markov property of the driver; independence and stationarity of increments play no role in the restriction step.
\end{remark}
...
\end{equation*}
Since $f_T(w - x)$ and $\Psi(\d w)$ do not depend on $(t, z)$, the operator passes through the integral defining $h_{tT}$: differentiation under the integral sign handles the local part and Fubini's theorem the jump part, both licensed by the envelope~\eqref{eq:harmonic-envelope}. Therefore
\begin{equation*}
...
It remains to upgrade the local martingale to a true martingale. The compensated jump integral is a priori only a local martingale; however, $\{M_t\}$ is a true martingale by Proposition~\ref{stochfiltlevyrandom}, and $\{\beta W_t\}$ is a true martingale; hence the purely discontinuous part $M_t - x - \beta W_t = \int_0^t\!\int_{\R\setminus\{0\}} y\,\widetilde{N}^{\xi}(\d s, \d y)$ is a difference of true martingales, and is therefore itself a true martingale.
