...
Our main contributions are as follows. First, we connect these processes to Rectified Flows, demonstrating that the conditional drift of such a bridge has the same functional form as the velocity field of rectified flow, and that this identity holds for every admissible driver. We prove that generalised Brownian random bridges can be explicitly represented as the sum of Rectified Flows plus standard Brownian bridges. This link places Rectified flow, random bridges, and \Schrodinger bridges on a common footing, a unifying perspective hitherto unavailable. In addition, we provide a non-anticipative semimartingale representation for the generalised \levy random bridge, carrying an explicit martingale component. This result is what makes non-Gaussian and jump drivers usable as transports rather than merely definable, taking a significant step forward towards implementing them for practical use cases. For a detailed account, we treat the Brownian and Poisson bridges as canonical continuous and discontinuous cases, and combine them in a jump-diffusion bridge. Through this journey, we verify explicit connections to Rectified Flows, generalise Tweedie's formula, and establish a temporal transport consistency under which every sub-bridge is again a transport between intermediate laws. Finally, we provide training and simulation algorithms equipped with numerical implementations, leaving the natural extension to discrete and mixed data to future work.
...
\end{proposition}
We prove and provide an explicit representation of the $(\P,\{\F_{t}^{\xib^{\boldsymbol{x}}}\}_{t\in\T})$-martingale $\{\mathbf{M}_t\}_{t\in\T}$ later in this section (Theorem~\ref{thm:explicit-decomp}). For now, denoting  $\langle \boldsymbol{M}_t^c \rangle$ as the continuous part and $\Delta \boldsymbol{M}_t = \boldsymbol{M}_t - \boldsymbol{M}_{t-}$ for all $t\in[0,T)$, the quadratic variation process of $\{\xib^{\boldsymbol{x}}_t\}_{t\in\T}$, written as $\{\langle\xib^{\boldsymbol{x}}_t\rangle\}_{t\in\T}$, is given by the following decomposition:
\begin{align}
\langle \xib^{\boldsymbol{x}}_t\rangle = \langle \boldsymbol{M}_t^c \rangle + \sum_{0 < s \leq t} (\Delta\boldsymbol{M}_s)^2. \notag
\end{align}
This continuous and jump split of the quadratic variation is made explicit by the decomposition of $\{\mathbf{M}_t\}_{t\in\T}$ in Theorem~\ref{thm:explicit-decomp} below, where $\langle \boldsymbol{M}^c\rangle$ is the Brownian contribution and $\sum(\Delta\boldsymbol{M})^2$ the Poisson and jump contribution.
...
\begin{remark}
The stochastic transport process $\{\xib_t\}_{t\in\T}$ is a generalised Ornstein-Uhlenbeck process driven by two $(\P,\{\mathcal{F}_{t}^{\xib}\}_{t\in\T})$-martingales $\{\mathbf{M}^{\xib}_t\}_{t\in\T}$ and $\{\bar{\mathbf{M}}^{\xib}_t\}_{t\in\T}$, where the martingale equilibrium process $\{\bar{\mathbf{M}}^{\xib}_t\}_{t\in\T}$ is itself a transport process.
\end{remark}
...
\subsection{Bridge-Modified \levy Measure}\label{app:modified-levy}
\label{app:explicit_M}

We work in one dimension for the explicit decomposition, the multivariate case following coordinatewise. Let $\{\xi^{x}_t\}_{t \in \T}$ be a $\delta_x$-initialised random bridge to $\Psi$ under action of the \levy driver $\{Z_t\}$ with $Z_0 = x$ (Definition~\ref{definitionRandomBridge}). This conditioning is implemented by a Doob $h$-transform of the driver's generator $\mathcal{A}$, with harmonic function
\begin{equation}\label{eq:h-app}
  h_{tT}(z) = \int_{\R} \frac{f_{T-t}(w - z)}{f_T(w - x)}\,\Psi(\d w),
  \qquad 0 \leq t < T,\; z \in \R,
\end{equation}
where the $f_T(w - x)$ in the denominator reflects the initial condition $Z_0 = x$ (cf.\ Corollary~\ref{levycorodensity} for the fixed-endpoint case). This is the integrated form of the harmonic function $h_{tT}(z;y) = f_{T-t}(y - z)$ from Appendix~\ref{app:resultsongeneralisedlevy} (line below~\eqref{eq:ratio}), averaged against the terminal law $\Psi$.
...
The function $h_{tT}$ defined in~\eqref{eq:h-app} is space-time harmonic with respect to the generator~\eqref{eq:generator}: for every $t < T$ and $z \in \R$,
...
\begin{itemize}
  \item For fixed $w$, the transition density $f_{T-t}(w - z)$ satisfies the backward Kolmogorov equation
    ...
    \end{equation*}
  \item Since $f_T(w - x)$ and $\Psi(\d w)$ are independent of $(t, z)$, the operator passes through the integral defining $h_{tT}$:
    \begin{equation*}
...
The Doob $h$-transform of the \levy generator with harmonic function $h_{tT}$ is of the form
...
\begin{proof}[Proof]
\begin{itemize}
  \item The Doob $h$-transform formula for the bridge generator (\cite[Theorem~4.2, eq.~1.2]{PalmowskiRolski2002}) reads
    \begin{equation}\label{eq:doob-h-formula}
      \widetilde{\mathcal{A}}f(x) \;=\; h_{tT}(x)^{-1}\,\mathcal{A}(f h_{tT})(x) \;-\; f(x)\,h_{tT}(x)^{-1}\,\mathcal{A}h_{tT}(x).
    \end{equation}
  ...
  \item Dividing by $h_{tT}(x)$ and using the identity
    ...
    \end{equation*}
    the integrand splits into three groups:
    \begin{align}
      \frac{J}{h_{tT}(x)}
      &= \int_{\R\setminus\{0\}}\bigl[f(x{+}y) - f(x) - yf'(x)\1_{|y|<1}\bigr]\,
...
Let $\{\xi^{x}_t\}_{t \in \T}$ be the $\delta_x$-initialised random bridge to $\Psi$ from Subsection~\ref{app:modified-levy}, with terminal random variable $Y \sim \Psi$ (so $\xi^{x}_T = Y$), satisfying $\E[|\xi^{x}_t|] < \infty$. The non-anticipative semimartingale representation of Proposition~\ref{stochfiltlevyrandom} gives
...
Under the assumptions above, the martingale $M_t$ in~\eqref{eq:semimartingale-app} admits the decomposition
...
\end{equation}
is the compensated Poisson random measure associated to the bridge dynamics, with bridge-modified compensator $\widetilde{\eta}$ given by Theorem~\ref{thm:modified-levy-app}.
\end{thm}
...
\begin{itemize}
  ...
  \item \emph{Continuous part.} The diffusion coefficient $\half\beta^2 f''(x)$ in~\eqref{eq:modified-generator-app} equals that of~\eqref{eq:generator}, so the continuous martingale part of $\xi^{x}_t$ has predictable quadratic variation $\langle M^c \rangle_t = \beta^2 t$, which is deterministic. By the \levy characterisation of Brownian motion \cite[Theorem~3.3.16]{KaratzasShreve1998}, there exists a standard Brownian motion $W_t$ with $M_t^c = \beta\,W_t$.
  \item \emph{Jump part.} The jump integral in~\eqref{eq:modified-generator-app} reads off the bridge-dynamics compensator of $N$ as $\frac{h_{sT}(\xi^{x}_s + y)}{h_{sT}(\xi^{x}_s)}\eta(\d y)\,\d s$, so the compensated jump martingale is $\int_0^t\!\int_{\R\setminus\{0\}} y\,\widetilde{N}^{\xi}(\d s, \d y)$ with $\widetilde{N}^{\xi}$ as in~\eqref{eq:bridge-compensator-app}.
...
The bridge drift admits two faces. Section~\ref{sec:semimartingale} gives it as the score of the
...
\begin{proposition}\label{prop:tweedie_gen}
    Let $\bfeta \sim g$ and $\boldsymbol{Z} \mid \bfeta \sim \text{GE}(h, \phi, F, \psi ; \bfeta)$, where $\nabla_{\bfz}F(\bfz) = \boldsymbol{1}c$ for some constant $c\neq 0$. Let $f$ be the marginal density of $\boldsymbol{Z}$. Then
    \begin{equation*}
        \mathbb{E}[\phi(\bfeta) \mid \boldsymbol{Z} = \boldsymbol{z}] = \left(\nabla_{\bfz} \log f(\bfz) - \E[\nabla_{\bfz} \log h(\bfz,\bfeta) \mid \boldsymbol{Z} = \boldsymbol{z} ]\right)c^{-1}.
    \end{equation*}
\end{proposition}

\begin{corollary}\label{prop:tweedie_gen_coro}
    Let $\bfeta \sim g$ and $\boldsymbol{Z} \mid \bfeta \sim \text{Exp}(h, \phi, F, \psi ; \bfeta)$, where $\nabla_{\bfz}F(\bfz) = \boldsymbol{1}$. Let $f$ be the marginal density of $\boldsymbol{Z}$. Then
    \begin{equation*}
...
Proofs are given in Appendix~\ref{app:tweedie-proofs}.
...
\end{align}
for any $t\in(0,T)$, given that $\boldsymbol{x} \leq \boldsymbol{z}  \leq \boldsymbol{y} \in \R^n$.
\end{lem}
\begin{proof}
...
The dynamics of $\{\boldsymbol{\xi}^{\boldsymbol{x}}_t\}_{t \in \mathbb{T}}$ has drift given by
    \begin{equation*}
        \frac{\mathbb{E}[\boldsymbol{Y} \mid \xib^{\boldsymbol{x}}_t ] - \boldsymbol{\xi}^{\boldsymbol{x}}_t}{T-t} = \frac{\nabla_{\boldsymbol{\xi}} \log p_t(\boldsymbol{\xi}^{\boldsymbol{x}}_t) - \E[\nabla_{\boldsymbol{\xi}} \log h(\boldsymbol{\xi}^{\boldsymbol{x}}_t,\boldsymbol{Y}) \mid \xib^{\boldsymbol{x}}_t ] - (\log(t) - \log(T-t))\boldsymbol{\xi}^{\boldsymbol{x}}_t}{(T-t)(\log(t) - \log(T-t))},
		\end{equation*}
where $p_t$ denotes the marginal density of $\boldsymbol{\xi}^{\boldsymbol{x}}_t$ for every $t\in[0, T)$. Hence,
		\begin{align}
		\mathbb{E}\left[\dd \boldsymbol{\xi}^{\boldsymbol{x}}_t \,|\, \xib^{\boldsymbol{x}}_{t} \right] = \left(\frac{\nabla_{\boldsymbol{\xi}} \log p_t(\boldsymbol{\xi}^{\boldsymbol{x}}_t) - \E[\nabla_{\boldsymbol{\xi}} \log h(\boldsymbol{\xi}^{\boldsymbol{x}}_t,\boldsymbol{Y}) \mid \xib^{\boldsymbol{x}}_t ] - (\log(t) - \log(T-t))\boldsymbol{\xi}^{\boldsymbol{x}}_t}{(T-t)(\log(t) - \log(T-t))}\right) \dd t,	 \notag
		\end{align}		
		for every $t\in(0, T)$.		
\end{corollary}
\begin{proof}
The result follows from Proposition \ref{prop:tweedie_gen} and Proposition \ref{poissonbridgegeneralisedexponential}, since $\nabla_{\bfz}F(\bfz) = \boldsymbol{1}$. The second part follows from (\ref{criticalconnection}), since $\{\mathbf{M}_t\}_{t\in\T}$ is a $(\P,\{\mathcal{F}_{t}^{\xib}\}_{t\in\T})$-martingale.
\end{proof}
...
\end{align}
for any $0\leq r < t < u \leq T$, and since the following limits are implicitly imposed by construct:
\begin{align}
\boldsymbol{\xi}^{\boldsymbol{x}}_r \sim \boldsymbol{\Phi}_r \,\, \text{where} \,\, \boldsymbol{\Phi}_r \overset{d}{\rightarrow} \boldsymbol{\Phi} \,\,\, \text{as} \,\,\, r \rightarrow 0 \,\,\,\, \text{and} \,\,\,\,
\boldsymbol{\xi}^{\boldsymbol{x}}_u \sim \boldsymbol{\Psi}_u \,\, \text{where} \,\, \boldsymbol{\Psi}_u \overset{d}{\rightarrow} \boldsymbol{\Psi} \,\,\, \text{as} \,\,\, u \rightarrow T, \label{twodistributionsubtransportspoisson}
\end{align}
each $\{\boldsymbol{\xi}^{\boldsymbol{x}}_t\}_{t \in [r,u]}$ defines a \emph{$\boldsymbol{\Phi}_r$-initialised random-bridge to $\boldsymbol{\Psi}_u$} for any $u\in(t,T]$. 
...
\label{app:imf}
\label{app:connection_with_imf}

We now place the Brownian random bridge training procedure within the framework of Iterative Markovian Fitting (IMF) introduced by \cite{shi2023diffusion}. IMF is an alternating projection scheme for computing \Schrodinger bridges. We begin by introducing the two constituent projections, following \cite{shi2023diffusion}.

Let $\mathcal{P}(\mathcal{C})$ denote the space of path measures on $\mathcal{C}([0,T], \R^n)$. We write $\mathcal{M} \subset \mathcal{P}(\mathcal{C})$ for the subset of Markov path measures associated with SDEs of the form $\dd\boldsymbol{X}_t = \{f(\boldsymbol{X}_t, t) + \boldsymbol{v}(\boldsymbol{X}_t, t)\}\dd t + \boldsymbol{\sigma}(\boldsymbol{X}_t, t)\dd\boldsymbol{B}_t$, where $\boldsymbol{v}, \boldsymbol{\sigma}$ are locally Lipschitz. For any $\mathbb{Q} \in \mathcal{M}$, we denote its diffusion bridge by $\mathbb{Q}_{|0,T}(\cdot \mid \boldsymbol{x}_0, \boldsymbol{x}_T)$, and for any joint distribution $\Pi_{0,T}$ over $\R^n \times \R^n$, we write $\Pi = \Pi_{0,T}\mathbb{Q}_{|0,T} \in \mathcal{P}(\mathcal{C})$ for the associated random bridge (\emph{mixture of bridges}) measure.

\begin{defn}[Reciprocal class and reciprocal projection {\cite[Definition~3]{shi2023diffusion}}]
\label{def:reciprocal}
A path measure $\Pi \in \mathcal{P}(\mathcal{C})$ is in the \emph{reciprocal class} $\mathcal{R}(\mathbb{Q})$ of $\mathbb{Q} \in \mathcal{M}$ if $\Pi = \Pi_{0,T}\,\mathbb{Q}_{|0,T}$. Given $\P \in \mathcal{P}(\mathcal{C})$, the \emph{reciprocal projection} is defined as
\begin{equation*}
    \mathrm{proj}_{\mathcal{R}(\mathbb{Q})}(\P) \;=\; \P_{0,T}\,\mathbb{Q}_{|0,T}, \label{eq:def_reciprocal_proj}
\end{equation*}
which retains the endpoint coupling of $\P$ but replaces the intermediate dynamics with the reference bridge $\mathbb{Q}_{|0,T}$.
\end{defn}

The reciprocal projection minimises the KL divergence to $\P$ over $\mathcal{R}(\mathbb{Q})$: if $\Pi^* = \mathrm{proj}_{\mathcal{R}(\mathbb{Q})}(\P)$, then $\Pi^* = \argmin_{\Pi \in \mathcal{R}(\mathbb{Q})} \mathrm{KL}(\P \| \Pi)$ \cite[Proposition~4]{shi2023diffusion}.

\begin{defn}[Markovian projection {\cite[Definition~1]{shi2023diffusion}}]
\label{def:markovian}
Assume that $\mathbb{Q} \in \mathcal{M}$ is associated with
\begin{equation*}
    \dd\boldsymbol{X}_t = f(\boldsymbol{X}_t, t)\dd t + \boldsymbol{\sigma}(\boldsymbol{X}_t, t)\dd\boldsymbol{B}_t,
\end{equation*}
and that for any $(\boldsymbol{x}_0, \boldsymbol{x}_T) \in \R^n$, the diffusion bridge $\mathbb{Q}_{|0,T}(\cdot \mid \boldsymbol{x}_0, \boldsymbol{x}_T)$ is associated with $(\boldsymbol{X}_t^{0,T})_{t \in [0,T]}$ given by
\begin{equation*}
    \dd\boldsymbol{X}_t^{0,T} = \{f(\boldsymbol{X}_t^{0,T}, t) + \boldsymbol{\sigma}(\boldsymbol{X}_t^{0,T}, t)^2 \nabla \log \mathbb{Q}_{T|t}(\boldsymbol{x}_T \mid \boldsymbol{X}_t^{0,T})\}\dd t + \boldsymbol{\sigma}(\boldsymbol{X}_t^{0,T}, t)\dd\boldsymbol{B}_t,
\end{equation*}
with $\boldsymbol{\sigma}(\cdot, \cdot) : \R^n \times [0, T] \to \R^{n \times n}$ a positive-definite diagonal matrix. Then, when it is well-defined, we introduce the \emph{Markovian projection} of $\Pi$, $\mathbb{M}^* = \mathrm{proj}_{\mathcal{M}}(\Pi) \in \mathcal{M}$, which is associated with the SDE
\begin{align}
    \dd\boldsymbol{X}_t^* &= \{f(\boldsymbol{X}_t^*, t) + \boldsymbol{v}^*(\boldsymbol{X}_t^*, t)\}\dd t + \boldsymbol{\sigma}(\boldsymbol{X}_t^*, t)\dd\boldsymbol{B}_t,\nonumber \\
    \boldsymbol{v}^*(\boldsymbol{x}, t) &= \boldsymbol{\sigma}(\boldsymbol{x}, t)^2\,\E_{\Pi_{T|t}}\!\left[\nabla \log \mathbb{Q}_{T|t}(\boldsymbol{X}_T \mid \boldsymbol{X}_t) \;\middle|\; \boldsymbol{X}_t = \boldsymbol{x}\right]. \label{eq:def_markovian_proj}
\end{align}
\end{defn}

The Markovian projection satisfies $\mathbb{M}^* = \argmin_{\mathbb{M} \in \mathcal{M}} \mathrm{KL}(\Pi \| \mathbb{M})$ and preserves all time-marginals: $\mathbb{M}^*_t = \Pi_t$ for every $t \in [0, T]$ \cite[Proposition~2]{shi2023diffusion}. In the Brownian case ($f = 0$, $\boldsymbol{\sigma}(\boldsymbol{x}, t) = \boldsymbol{\sigma}$ constant), the bridge $\mathbb{Q}_{|0,T}(\cdot \mid \boldsymbol{x}, \boldsymbol{y})$ is a Brownian bridge with drift $(\boldsymbol{y} - \xib_t)/(T-t)$, so the Markovian projection drift~\eqref{eq:def_markovian_proj} reduces to
\begin{equation}
...
IMF \cite[Section~3.2]{shi2023diffusion} alternates between these projections, constructing a sequence $(\P^n)_{n \in \N}$ via
\begin{equation}
    \P^{2n+1} = \mathrm{proj}_{\mathcal{M}}(\P^{2n}), \qquad \P^{2n+2} = \mathrm{proj}_{\mathcal{R}(\mathbb{Q})}(\P^{2n+1}), \label{eq:imf_iteration}
\end{equation}
with $\P^0 \in \mathcal{R}(\mathbb{Q})$ satisfying $\P^0_0 = \boldsymbol{\Phi}$ and $\P^0_T = \boldsymbol{\Psi}$. Both marginals are preserved at every iteration: $\P^n_0 = \boldsymbol{\Phi}$ and $\P^n_T = \boldsymbol{\Psi}$ for all $n$ \cite[Theorem~8]{shi2023diffusion}. The sequence converges to the unique \Schrodinger bridge $\P^{\mathrm{SB}}$, which is the solution to the \Schrodinger bridge problem: find the path measure closest to the reference process $\mathbb{Q}$ in KL divergence subject to boundary constraints,
\begin{equation}
    \P^{\mathrm{SB}} = \argmin_{\P \in \mathcal{P}(\mathcal{C})} \left\{ \mathrm{KL}(\P \| \mathbb{Q}) \;:\; \P_0 = \boldsymbol{\Phi},\; \P_T = \boldsymbol{\Psi} \right\}. \label{eq:sb_problem}
\end{equation}
...
A practical advantage of this single-iteration random bridge regime is that it avoids the \emph{simulation-inference mismatch} identified by \cite{peluchetti2023diffusion}. In iterative IPF-based methods such as DSB \cite{debortoli2021diffusion}, each iterate $\P^{n+1}$ is obtained by simulating trajectories from the \emph{previous} iterate $\P^n$ and regressing a new drift against them. Since $\P^n$ and $\P^{n+1}$ have different initial distributions, training data is drawn from a distribution that does not match the model being learned, leading to error accumulation across iterations. In contrast, the random bridge training samples directly from the known reference bridge $\mathbb{Q}_{|0,T}$ via an explicit reciprocal projection, so no learned drift is used to generate training data. This ensures exact consistency between the training distribution and the model at every stage.

\begin{remark}[General \levy reference]
The \Schrodinger problem is defined relative to an arbitrary reference path measure $R$, not only
Wiener measure, and its solution is Markov whenever $R$ is \cite{leonard2014survey}. Taking $R$ to be a
\levy law, the reciprocal class is built from the \levy random bridges of
Section~\ref{sec:framework}, and the Markovian-projection drift is the harmonic-function score
$\beta^2\partial_x\log h_{tT}$ of Section~\ref{sec:semimartingale}, the engine this paper supplies. The
Brownian reference is the special case that additionally connects to entropic optimal transport, the
quadratic Benamou-Brenier reading being specific to the Wiener case. For a general \levy reference one
obtains a bona-fide \levy-\Schrodinger problem. We establish the formulation and the single IMF
iteration constructively, and leave full IMF convergence beyond the diffusion case as a conjecture.
\end{remark}
...
\end{align}
    ...
    Since $g(\bfeta \mid \bfz)$ is a density function, we must have
		...
		\end{align}
		Moving the differential-operator inside the integral and inserting (\ref{eq:new_exp_family}), we get
	 \begin{align}
...
& = \nabla_{\bfz}\log(f(\bfz)) - \E[\nabla_{\bfz}\log(h(\bfz,\bfeta)) \mid \boldsymbol{Z} = \boldsymbol{z} ], \notag
\end{align}
and the statement follows since $\nabla_{\bfz}F(\bfz) = \boldsymbol{1}c$, which carries over to the right-hand side of the expression with $c\neq 0$.
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{prop:tweedie_gen_coro}]
    ...
    \end{equation*}
		which follows from the moment-generating function of $\phi(\bfeta)$, given that $\nabla_{\bfz}F(\bfz) = \boldsymbol{1}$.
\end{proof}
