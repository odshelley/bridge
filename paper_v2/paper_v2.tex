...
This section develops the framework in five movements. Subsection~\ref{sec:framework}.1 defines random bridges and their conditioning; Subsection~\ref{sec:semimartingale} states our first main result, the explicit pathwise representation of Theorem~\ref{thm:main-rep}; Subsections~\ref{sec:rf-connection} and~\ref{sec:temporal-consistency} draw its two structural consequences, the Rectified Flow correspondence and the temporal transport consistency; and Subsections~\ref{sec:modified-levy} and~\ref{sec:explicit-decomp} supply the proof through the bridge-modified \levy measure.
...
The representation of Theorem~\ref{thm:main-rep} holds $\P$-almost surely on the canonical bridge space, so its differential form may be manipulated pathwise. Taking the conditional expectation of that differential form yields the following:
\begin{align}
...
\end{align}
since the martingale part of Theorem~\ref{thm:main-rep} has vanishing conditional increments. The conditional expectation provided in (\ref{criticalconnection}) will have a crucial stance for the rest of this paper towards connecting Rectified Flows and \Schrodinger Bridges with \levy random bridges.
\begin{lem}
...
\begin{prop}[Rectified Flow correspondence]
\label{coredynamicconnection}
Let $\{\xib^{\boldsymbol{x}}_t\}_{t\in\T}$ be a $\boldsymbol{\Phi}$-initialised random bridge to $\boldsymbol{\Psi}$ under \emph{any} \levy driver with $\E[|\xib^{\boldsymbol{x}}_t|] < \infty$. Then:
\begin{enumerate}
    \item[(i)] \emph{(Dynamical layer.)} The conditional drift of the bridge coincides in functional form with the optimal Rectified Flow velocity field,
    \begin{align}
        \E\left[\dd \xib^{\boldsymbol{x}}_{t} \,|\, \xib^{\boldsymbol{x}}_{t} \right] = \frac{\E\left[ \boldsymbol{Y} \, | \, \xib^{\boldsymbol{x}}_t \right]  - \xib^{\boldsymbol{x}}_t }{T-t}\dd t \, = \, \boldsymbol{v}^*(\xib^{\boldsymbol{x}}_t, t)\,\dd t, \notag
    \end{align}
    where $\boldsymbol{v}^*$ is the velocity field of (\ref{odemodelbaseconnection}).
    \item[(ii)] \emph{(Statistical layer.)} The mean of the pinned bridge is the Rectified Flow interpolant, $\E[\boldsymbol{\xi}^{\boldsymbol{x},\boldsymbol{y}}_t] = \boldsymbol{R}^{\boldsymbol{x,y}}_t$ for every $t \in \T$.
\end{enumerate}
\end{prop}
\begin{proof}
Part (i) is (\ref{criticalconnection}), which rests on Theorem~\ref{thm:main-rep} and holds for any \levy driver satisfying the stated moment condition, compared against the target-pull form (\ref{eq:rf_target_pull}) of the Rectified Flow velocity. Part (ii) follows from (\ref{coreexpectationbridge}).
\end{proof}
Proposition~\ref{coredynamicconnection} is the sense in which generalised \levy random bridges are stochastic extensions of Rectified Flows: neither layer depends on the driver being Brownian.
Proposition \ref{coredynamicconnection} paves a path that takes us well beyond Brownian motion frameworks, as commonly seen in the literature, and allows us to take a step forward in unifying different approaches. Finally, in the spirit of (\ref{purebridgeconstruct}), we define the following causal (non-anticipative) process:
\begin{align}
...
\begin{equation}
    ...
    \end{equation}
for any $t\in\T$, restating part (ii) of Proposition~\ref{coredynamicconnection} in the pinned notation.
\begin{remark}
...
Let $\{\xib^{\boldsymbol{x}}_t\}_{t\in\T}$ be a c\`adl\`ag $\boldsymbol{\Phi}$-initialised random bridge to $\boldsymbol{\Psi}$ under a \levy driver $\{\boldsymbol{Z}_t\}_{t\in\T}$, with endpoint coupling absolutely continuous with respect to the law of $(\boldsymbol{Z}_0, \boldsymbol{Z}_T)$. For $0\le r<u\le T$, let $\boldsymbol{\Phi}_r$ and $\boldsymbol{\Psi}_u$ denote the laws of $\xib^{\boldsymbol{x}}_r$ and $\xib^{\boldsymbol{x}}_u$ respectively. Then $\{\xib^{\boldsymbol{x}}_t\}_{t\in[r,u]}$ is a $\boldsymbol{\Phi}_r$-initialised random bridge to $\boldsymbol{\Psi}_u$ under the restricted driver $\{\boldsymbol{Z}_t\}_{t\in[r,u]}$, and
\begin{align}
\boldsymbol{\Phi}_r\xrightarrow{d}\boldsymbol{\Phi} \,\,\, \text{as} \,\,\, r \downarrow 0
\qquad \text{and} \qquad
\boldsymbol{\Psi}_u\xrightarrow{d}\boldsymbol{\Psi} \,\,\, \text{as} \,\,\, u \uparrow T. \notag
\end{align}
As a consequence, the drift re-anchors to any intermediate horizon,
\begin{align}
...
The proof rests on a screening property of Markov processes: conditionally on the states at two times, the path between them is independent of the path outside them.

\begin{lem}[Screening by intermediate endpoints]
\label{lem:screening}
Let $\{\boldsymbol{Z}_t\}_{t\in\T}$ be an $\R^n$-valued Markov process with regular transition kernels. Fix $0\le r<t_1<\cdots<t_m<u\le T$. Then, for every bounded Borel function $F$,
\begin{align}
\E\!\left[
F(\boldsymbol{Z}_{t_1},\ldots,\boldsymbol{Z}_{t_m})
\mid
\boldsymbol{Z}_0,\boldsymbol{Z}_r,\boldsymbol{Z}_u,\boldsymbol{Z}_T
\right]
=
\E\!\left[
F(\boldsymbol{Z}_{t_1},\ldots,\boldsymbol{Z}_{t_m})
\mid
\boldsymbol{Z}_r,\boldsymbol{Z}_u
\right]. \notag
\end{align}
\end{lem}

\begin{proof}
Let $P_{s,t}$ denote the transition kernel of $\{\boldsymbol{Z}_t\}_{t\in\T}$ and let $K^{\boldsymbol{t}}_{r,u}((\boldsymbol{a},\boldsymbol{b}),\cdot)$ be a regular conditional law of $(\boldsymbol{Z}_{t_1},\ldots,\boldsymbol{Z}_{t_m})$ given $(\boldsymbol{Z}_r,\boldsymbol{Z}_u)=(\boldsymbol{a},\boldsymbol{b})$, obtained by disintegrating the product
$P_{r,t_1}(\boldsymbol{a},\dd \boldsymbol{z}_1) P_{t_1,t_2}(\boldsymbol{z}_1,\dd \boldsymbol{z}_2)\cdots P_{t_m,u}(\boldsymbol{z}_m,\dd \boldsymbol{b})$
with respect to $P_{r,u}(\boldsymbol{a},\dd \boldsymbol{b})$. For bounded Borel functions $G_0,G_r,G_u,G_T$, writing the joint law of $(\boldsymbol{Z}_0, \boldsymbol{Z}_r, \boldsymbol{Z}_{t_1}, \ldots, \boldsymbol{Z}_{t_m}, \boldsymbol{Z}_u, \boldsymbol{Z}_T)$ as the product of transition kernels and integrating out the interior variables gives
\begin{align}
\E\!\left[
G_0(\boldsymbol{Z}_0)G_r(\boldsymbol{Z}_r)
F(\boldsymbol{Z}_{t_1},\ldots,\boldsymbol{Z}_{t_m})
G_u(\boldsymbol{Z}_u)G_T(\boldsymbol{Z}_T)
\right]
=
\E\!\left[
G_0(\boldsymbol{Z}_0)G_r(\boldsymbol{Z}_r)
G_u(\boldsymbol{Z}_u)G_T(\boldsymbol{Z}_T)\,
(K^{\boldsymbol{t}}_{r,u}F)(\boldsymbol{Z}_r,\boldsymbol{Z}_u)
\right], \notag
\end{align}
where $(K^{\boldsymbol{t}}_{r,u}F)(\boldsymbol{a},\boldsymbol{b}) = \int F\,\dd K^{\boldsymbol{t}}_{r,u}((\boldsymbol{a},\boldsymbol{b}),\cdot)$. This identifies both conditional expectations in the statement with $(K^{\boldsymbol{t}}_{r,u}F)(\boldsymbol{Z}_r,\boldsymbol{Z}_u)$. The boundary cases $r=0$ and $u=T$ follow by omitting the corresponding exterior variable.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:temporal-consistency}]
Let $\boldsymbol{\Gamma}_{r,u}$ denote the joint law of $(\xib^{\boldsymbol{x}}_r,\xib^{\boldsymbol{x}}_u)$; its marginals are $\boldsymbol{\Phi}_r$ and $\boldsymbol{\Psi}_u$. Fix $r<t_1<\cdots<t_m<u$ and a bounded Borel function $F$. By Definition~\ref{definitionRandomBridge}, applied to the finite set of times $\{r,t_1,\ldots,t_m,u\}$, the conditional finite-dimensional law of the bridge given $(\xib^{\boldsymbol{x}}_0,\xib^{\boldsymbol{x}}_T)$ agrees with that of the driver given $(\boldsymbol{Z}_0,\boldsymbol{Z}_T)$. Lemma~\ref{lem:screening} therefore yields
\begin{align}
\E\!\left[
F(\xib^{\boldsymbol{x}}_{t_1},\ldots,\xib^{\boldsymbol{x}}_{t_m})
\mid
\xib^{\boldsymbol{x}}_r,\xib^{\boldsymbol{x}}_u
\right]
=
\E\!\left[
F(\boldsymbol{Z}_{t_1},\ldots,\boldsymbol{Z}_{t_m})
...
\right] \notag
\end{align}
for $\boldsymbol{\Gamma}_{r,u}$-almost every endpoint pair. Taking $F$ to be indicators of lower orthants gives precisely condition (2) of Definition~\ref{definitionRandomBridge} on the interval $[r,u]$, while condition (1) holds by construction of $\boldsymbol{\Gamma}_{r,u}$. Hence the restriction is a $\boldsymbol{\Phi}_r$-initialised random bridge to $\boldsymbol{\Psi}_u$.

For the endpoint limits, right-continuity of the c\`adl\`ag paths at zero gives $\xib^{\boldsymbol{x}}_r\to\xib^{\boldsymbol{x}}_0$ $\P$-almost surely as $r\downarrow0$, hence $\boldsymbol{\Phi}_r\xrightarrow{d}\boldsymbol{\Phi}$. For the terminal endpoint, a \levy process has no fixed times of discontinuity, so $\P(\Delta\boldsymbol{Z}_T\ne0)=0$, and therefore $q(\boldsymbol{a},\boldsymbol{b}) = \P(\Delta\boldsymbol{Z}_T\ne0\mid \boldsymbol{Z}_0=\boldsymbol{a},\boldsymbol{Z}_T=\boldsymbol{b}) = 0$ for almost every endpoint pair under the law of $(\boldsymbol{Z}_0, \boldsymbol{Z}_T)$. Since the endpoint coupling $\boldsymbol{\Gamma}$ is absolutely continuous with respect to that law,
\begin{align}
\P(\Delta\xib^{\boldsymbol{x}}_T\ne0)
=
\int q(\boldsymbol{a},\boldsymbol{b})\,
\boldsymbol{\Gamma}(\dd\boldsymbol{a},\dd\boldsymbol{b})
=0. \notag
\end{align}
Thus $\xib^{\boldsymbol{x}}_{T-}=\xib^{\boldsymbol{x}}_T$ $\P$-almost surely, so $\xib^{\boldsymbol{x}}_u\to\xib^{\boldsymbol{x}}_T$ almost surely as $u\uparrow T$, and $\boldsymbol{\Psi}_u\xrightarrow{d}\boldsymbol{\Psi}$. Finally, the linear interpolation identity $\E[\xib^{\boldsymbol{x}}_u\mid\xib^{\boldsymbol{x}}_t] = \frac{u-t}{T-t}\E[\xib^{\boldsymbol{x}}_T\mid\xib^{\boldsymbol{x}}_t] + \frac{T-u}{T-t}\xib^{\boldsymbol{x}}_t$ of \cite[Corollary~2.10]{hoylehughstonmacrina_2011}, together with $\boldsymbol{Y}=\xib^{\boldsymbol{x}}_T$ $\P$-almost surely, rearranges to the drift re-anchoring identity.
\end{proof}

\begin{remark}
The absolute-continuity hypothesis on the endpoint coupling holds throughout the $h$-transform framework of Section~\ref{sec:modified-levy}, where $\boldsymbol{\Psi}$ is absolutely continuous with respect to the terminal law of the driver and the coupling is built from the corresponding conditional kernels. It excludes pinned bridges with atomic terminal laws, such as the classical bridge to $\boldsymbol{\delta}_{\boldsymbol{y}}$, for which the terminal limit must be verified directly from the bridge kernel; for the Brownian and Poisson pinned bridges of Sections~\ref{sec:brownian} and~\ref{sec:poisson} this is immediate, by path continuity in the former case and by the non-atomicity of the conditional jump-time laws in the latter. The screening lemma itself uses only the Markov property of the driver; independence and stationarity of increments play no role in the restriction step.
\end{remark}

The drift re-anchoring admits a dynamical reading: the conditional target velocity can be understood as an appropriately time-adjusted mean-reversion on the $\mathcal{L}^2$-best-estimate of \emph{any} future state of the random bridge $\{\boldsymbol{\xi}^{\boldsymbol{x}}_t\}_{t \in \mathbb{T}}$. Combining (\ref{nonanticipativereplevyrandombridge}) with the re-anchoring identity of Proposition~\ref{prop:temporal-consistency}, we have
\begin{align}
...
\begin{conj}[Temporal consistency of Rectified Flows]
\label{conj:rf-consistency}
...
\end{equation}
for any $u\in(t,T]$. Establishing this relation is beyond the scope of this paper; we return to it in Section~\ref{sec:conclusion}.
\end{conj}
...
\end{equation}
together with the absolute double-integral bound for the nonlocal part,
\begin{equation}\label{eq:harmonic-fubini}
  \int_{\R} \int_{\R\setminus\{0\}} \sup_{(t,z) \in K} \frac{\bigl|f_{T-t}(w - z - y) - f_{T-t}(w - z) - y\,\partial_z f_{T-t}(w - z)\,\1_{|y|<1}\bigr|}{f_T(w - x)}\,\eta(\d y)\,\Psi(\d w) < \infty.
\end{equation}
...
\begin{proof}
For fixed $w \in \R$, the transition density satisfies the backward Kolmogorov equation
\begin{equation*}
  (\partial_t + \mathcal{A}_z)\,f_{T-t}(w - z) = 0, \qquad t < T.
\end{equation*}
Since $f_T(w - x)$ and $\Psi(\d w)$ do not depend on $(t, z)$, the operator passes through the integral defining $h_{tT}$: differentiation under the integral sign handles the local part, licensed by the envelope~\eqref{eq:harmonic-envelope}, and Fubini's theorem handles the jump part, licensed by the absolute bound~\eqref{eq:harmonic-fubini}. Therefore
\begin{equation*}
  (\partial_t + \mathcal{A}_z)\,h_{tT}(z)
  = \int_{\R} \frac{(\partial_t + \mathcal{A}_z)\,f_{T-t}(w - z)}{f_T(w - x)}\,\Psi(\d w)
  = 0,
\end{equation*}
which is~\eqref{eq:harmonic-app}.
\end{proof}

\begin{thm}
\label{thm:modified-levy-app}
Assume that $h_{tT}$ is of class $C^{1,2}$ on $[0,T) \times \R$, strictly positive, a good function for the space-time generator $\partial_t + \mathcal{A}$ in the sense of \cite[Section~4]{PalmowskiRolski2002}, meaning that $h_{tT}$ and $1/h_{tT}$ preserve the domain, $\mathcal{D}((\partial_t + \mathcal{A})^{h}) = \mathcal{D}(\partial_t + \mathcal{A})$, and that for every $(t,x) \in [0,T) \times \R$,
...
\begin{proof}
The harmonic function $h_{tT}$ is time-dependent, so the $h$-transform must be taken for the space-time process $(t, Z_t)$, whose generator is $\partial_t + \mathcal{A}$ and for which $h$ is harmonic by Proposition~\ref{prop:harmonic-app}; the positivity and martingale hypotheses of \cite[Theorem~4.2]{PalmowskiRolski2002} hold by Proposition~\ref{prop:lrb-change-of-measure}, and the construction of bridges as space-time Doob $h$-transforms is classical \cite{FitzsimmonsPitmanYor1993}. For a time-independent test function $f$, the space-time transform gives
...
We now apply~\eqref{eq:generator} to $g = fh_{tT}$. The jump contribution to $\mathcal{A}(fh_{tT})$ is
    \begin{equation*}
      J = \int_{\R\setminus\{0\}}\bigl[f(x{+}y)\,h_{tT}(x{+}y)
        ...
        \1_{|y|<1}\bigr]\,\eta(\d y).
    \end{equation*}
Dividing by $h_{tT}(x)$ and using the identity
    \begin{equation*}
    ...
    \end{align}
Equation~\eqref{eq:jump-mod} is the jump integral with modified \levy measure $\frac{h_{tT}(x+y)}{h_{tT}(x)}\eta(\d y)$, giving~\eqref{eq:modified-levy-measure-app}. For the local terms, the Leibniz rule gives $(fh_{tT})' = f'h_{tT} + fh_{tT}'$ and $(fh_{tT})'' = f''h_{tT} + 2f'h_{tT}' + fh_{tT}''$. We substitute these into $\mathcal{A}(fh_{tT})/h_{tT}$ and collect the coefficients of $f^{(k)}(x)$ across the drift, diffusion, and jump expansions, the last contributed by~\eqref{eq:jump-mod},~\eqref{eq:drift-correction} and~\eqref{eq:zero-coefficient}.

Only the diffusion expansion contributes to the coefficient of $f''(x)$, giving $\half\beta^2$; the diffusion coefficient is unchanged. For the coefficient of $f'(x)$, we combine $\alpha$ from the drift expansion, the cross-term $\beta^2\,h_{tT}'(x)/h_{tT}(x)$ from the diffusion expansion, and~\eqref{eq:drift-correction}:
    \begin{equation*}
      \alpha + \beta^2\,\frac{h_{tT}'(x)}{h_{tT}(x)} + \int_{|y|<1} y\,\Bigl[\frac{h_{tT}(x+y)}{h_{tT}(x)} - 1\Bigr]\,\eta(\d y) \;=\; \widetilde{\alpha}(t,x),
    \end{equation*}
    matching~\eqref{eq:modified-drift-app}. For the coefficient of $f(x)$ in $h_{tT}^{-1}\mathcal{A}(f h_{tT})$, we combine $\alpha\,h_{tT}'(x)/h_{tT}(x)$ from the drift expansion, $\half\beta^2\,h_{tT}''(x)/h_{tT}(x)$ from the diffusion expansion, and~\eqref{eq:zero-coefficient}:
    \begin{equation*}
    ...
    \end{equation*}
The remaining jump integral $\int [f(x+y) - f(x) - yf'(x)\1_{|y|<1}]\,\frac{h_{tT}(x+y)}{h_{tT}(x)}\,\eta(\d y)$ from~\eqref{eq:jump-mod} is the bridge-modified jump term, completing~\eqref{eq:modified-generator-app}.
\end{proof}

\begin{coro}[Score form of the bridge drift]
\label{cor:score-drift}
...
\begin{align}
h(\boldsymbol{z},\boldsymbol{y}) = \prod_{i=1}^n {{y^{(i)} - x^{(i)}}\choose{z^{(i)} - x^{(i)}}} \exp\left(\log\left(\frac{t}{T-t} \right) \boldsymbol{1}^{\tp}(\boldsymbol{z} - \boldsymbol{x}) - \log\left(\frac{t}{T-t} \right)\boldsymbol{y} \cdot (\boldsymbol{z} - \boldsymbol{x}) \right), \notag
\end{align}
and
\begin{align}
\phi(\boldsymbol{y}) = \log\left(\frac{t}{T-t} \right)\boldsymbol{y}, \,\, F(\boldsymbol{z}) = \boldsymbol{z} - \boldsymbol{x}, \,\, \psi(\boldsymbol{y}) = -\log\left(\frac{T-t}{T}\right) \boldsymbol{1}^{\tp}(\boldsymbol{y} - \boldsymbol{x}) \notag
\end{align}
for lattice points $\boldsymbol{z}, \boldsymbol{y} \in \boldsymbol{x} + \mathbb{N}_0^n$ with $\boldsymbol{z} \leq \boldsymbol{y}$ componentwise.
\end{proposition}
...
&= h(\boldsymbol{z},\boldsymbol{y}) \exp\left(\log\left(\frac{t}{T-t} \right)\boldsymbol{y} \cdot (\boldsymbol{z} - \boldsymbol{x}) + \log\left(\frac{T-t}{T}\right) \boldsymbol{1}^{\tp}(\boldsymbol{y} - \boldsymbol{x}) \right) \notag
\end{align}
where we defined the exponential form
\begin{align}
v(\boldsymbol{z},\boldsymbol{y}) = \exp\left(\log\left(\frac{t}{T-t} \right) \boldsymbol{1}^{\tp}(\boldsymbol{z} - \boldsymbol{x}) - \log\left(\frac{t}{T-t} \right)\boldsymbol{y} \cdot (\boldsymbol{z} - \boldsymbol{x}) \right), \notag
\end{align}
